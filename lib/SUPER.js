...
function sendEvent(node, eventName) {
	...
	if (_eventName.match(/^mouse|click$/)) {
		...
		event = new MouseEvent(_eventName, {ctrlKey: parameters.modifierKeys.indexOf("ctrl") > -1, altKey: parameters.modifierKeys.indexOf("alt") > -1, shiftKey: parameters.modifierKeys.indexOf("shift") > -1, metaKey: parameters.modifierKeys.indexOf("meta") > -1});
	}
	else {
