...
function massMergeGUI() {
	...
	MMRdiv.appendChild(createTag("p", {}, "Herebelow, you can shift the alignment of local and remote tracklists."));
		...
		if (this.status < 200 || this.status >= 400) {
			...
			if (!remoteRelease.disc && releaseWithoutARs.match(/<tbody style="display:none"><\/tbody>/)) {
				...
				for (var m = 0; m < mediums.length; m++) {
					var current_medium = mediums[m].match(/<th colSpan="\d+">.+?disc\/(\d+)#.+?>([^>\d]+)(?:\d+)?<\/a>/);
					if (current_medium) {
						current_medium = current_medium[1] + " (" + current_medium[2].replace(/&quot;/, "ʺ").trim() + "): ";
