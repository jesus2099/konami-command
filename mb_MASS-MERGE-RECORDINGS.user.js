...
// @exclude      *.org/release/*/aliases
// @exclude      *.org/release/*/annotations
// @exclude      *.org/release/*/change-quality
// @exclude      *.org/release/*/collections
// @exclude      *.org/release/*/cover-art
// @exclude      *.org/release/*/delete
// @exclude      *.org/release/*/details
// @exclude      *.org/release/*/discids
// @exclude      *.org/release/*/edit*
// @exclude      *.org/release/*/open_edits
...
function loadReleasePage() {
	...
	xhr.addEventListener("load", function(event) {
		if (this.status == 200) {
			...
			var releaseAC = releaseWithoutARs.match(/<p class="subheader">[\s\S]*?<span class="prefix">~<\/span>(?: <!-- -->)?[\s\w]+ (?:<span class="mp">)?(<a href="\/artist\/.+)(?:<\/span>)?[\s\S]+?<span class="small">/);
//TODO: Cleanup when beta comes live: `an>(?: <!-- -->)?[\s\w]+ (<a` → `an> <!-- -->[\s\w]+ (<a`.
