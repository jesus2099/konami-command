...
if (j2sets.SERVER_SWITCH) {
	...
	if (langMenu) {
		...
		menu.addEventListener("click", function(event) {
			stop(event);
			for (var openMenus = document.querySelectorAll(".header ul.menu li.fake-active"), m = 0; m < openMenus.length; m++) {
			...
			if (this.lastChild.style.getPropertyValue("left").match(/inherit|-\d{1,4}px/)) {
				...
				this.lastChild.style.setProperty("left", "-" + (ulStyle.getPropertyValue("width").match(/\d+/) - self.getComputedStyle(this).getPropertyValue("width").match(/\d+/) + 2 * ulStyle.getPropertyValue("padding").match(/\d+/)) + "px");
			}
		});
