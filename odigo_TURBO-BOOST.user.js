...
if (location.pathname.match(/ivr-designer/)) {

	////// From odigo-ivr-designer_TURBO-BOOST //////

	waitForElement("input#keyword", function(element) { element.focus(); });

	switch (self.location.pathname) {

		// Homepage, application list
		...
		case "/application.html":
			...
			setInterval(function() {
				...
				// Go to List View by default (only once)
	...
	////// From odigo-routing_TURBO-BOOST //////
	...
	function backgroundTabIcons(event) {
		...
		if (event.target.matches("img.iconModify, img.iconView") && event.detail === 1) {
